...
import com.library.modules.sys.service.RoleService;
...
public class UserController {
    ...
    public ModelAndView form(@PathVariable("deptId") Integer deptId, @PathVariable("id") Integer id) {
        ...
        modelAndView.addObject("cardTypeList", dictService.getDictByDictValue(DictConstant.CARD_TYPE));
    ...
    @GetMapping("/vipList/{roleId}")
    public PageResult<SysUser> userlistByRole(@PathVariable("roleId") Integer roleId,@RequestParam(name = "keyword",required = false) String keyword) {
        List<SysUser> users = userService.getUserListByRoleId(roleId,keyword);
        return new PageResult(new PageInfo<SysUser>(users)).setCode(0);
    }
