;layui.define(["table", "form","tree","treeSelect","eleTree"], function (t) {
    ...
    table.render({
        ...
        cols: [[
            ...
            {field: "cardTypeName", title: "图书证类型",width: 120},
            ...
            success: function(layero, index){
                admin.req({
                    ...
                    url: "/sys/user/view/"+e.id,
                        ...
                        div.find("select[name='city']").val(data.city);
                        div.find("select[name='cardType']").val(data.cardType);

                        div.find("input[name='sex'][value=\'"+data.sex+"\']").prop("checked",true);
                        div.find("select[name='roleId']").val(data.roleId);
                    }
                });
            }
        })
